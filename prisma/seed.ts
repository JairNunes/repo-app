...
async function main() {
  ...
  const services = await Promise.all([
    prisma.service.create({ data: { name: 'Troca de Óleo', priceCents: 15000 } }),
    prisma.service.create({ data: { name: 'Alinhamento e Balanceamento', priceCents: 12000 } }),
    prisma.service.create({ data: { name: 'Revisão de Freios', priceCents: 25000 } }),
    prisma.service.create({ data: { name: 'Troca de Correia Dentada', priceCents: 45000 } }),
    prisma.service.create({ data: { name: 'Diagnóstico Eletrônico', priceCents: 8000 } }),
  ...
  const parts = await Promise.all([
    prisma.part.create({ data: { name: 'Óleo Motor 5W30 (1L)', priceCents: 4500, stockQuantity: 50 } }),
    prisma.part.create({ data: { name: 'Filtro de Óleo', priceCents: 3500, stockQuantity: 30 } }),
    ...
    prisma.part.create({ data: { name: 'Vela de Ignição (unidade)', priceCents: 2500, stockQuantity: 60 } }),
  ...
  const customers = await Promise.all([
    prisma.customer.create({ data: { name: 'João da Silva', document: '12345678901' } }),
  ...
  // 6. Service Orders (different statuses for demo)

  // OS 1 - Received (just created, ready for diagnosis)
  const so1 = await prisma.serviceOrder.create({
    data: {
      ...
      services: {
        create: [
          { serviceId: services[0].id, quantity: 1, unitPriceCentsSnapshot: 15000 },
        ],
      },
      parts: {
        create: [
          { partId: parts[0].id, quantity: 1, unitPriceCentsSnapshot: 4500 },
        ],
      },
    },
  });
  ...
  const so2 = await prisma.serviceOrder.create({
    data: {
      ...
      services: {
        create: [
          { serviceId: services[2].id, quantity: 1, unitPriceCentsSnapshot: 25000 },
        ],
      },
      parts: {
        create: [
          { partId: parts[3].id, quantity: 1, unitPriceCentsSnapshot: 12000 },
        ],
      },
    },
  });
  ...
  const so3 = await prisma.serviceOrder.create({
    data: {
      ...
      services: {
        create: [
          { serviceId: services[3].id, quantity: 1, unitPriceCentsSnapshot: 45000 },
        ],
      },
      parts: {
        create: [
  ...
  console.log('\nService Orders:');
  console.log(`  ${so1.id} - Received      (João/Corolla)`);
  console.log(`  ${so2.id} - InDiagnosis   (Maria/Gol)`);
  console.log(`  ${so3.id} - AwaitingApproval (Carlos/Argo)`);
  console.log(`  ${so4.id} - InExecution   (João/Civic)`);

  console.log('\n=== Seed completed! ===');
  console.log(`\nLogin: ${adminEmail} / ${adminPassword}`);
  console.log('Customers: João (12345678901), Maria (98765432100), Carlos (11122233344)');
}
